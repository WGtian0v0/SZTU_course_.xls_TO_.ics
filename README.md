# 将技术大学的课表的.xls文件转换成.ics日历文件
![Static Badge](https://img.shields.io/badge/python-3.12.7-blue?logo=python&logoColor=yellow)

本人有列计划的习惯，但是由于系统日历无法显示课表，只能一个个手动添加。市面上的其他方案要么不够完善，要么收费，无奈之下遂写了这个程序。

这个程序可以将深圳技术大学的学生课表转换成.ics日历文件，并支持爬取教务系统的课表，方便导入到手机或其他设备的日历中。

（如果教务系统课表有变动，要手动再添加一遍哦＜(´⌯  ̫⌯`)＞）
## 用法：
### 1.下载[Python 3.12.7](https://www.python.org/downloads/release/python-3127/)或Anaconda创建Python 3.12环境
> 安装官方python时注意勾选“Add Python 3.12 to PATH”

### 2.下载项目文件，解压。
>按需要更改[source/config.py](source/config.py)中的配置项  

| 配置名               | 类型     | 说明           | 默认值                                                                                                                                      | 备注                   |
|-------------------|--------|--------------|------------------------------------------------------------------------------------------------------------------------------------------|----------------------|
| `SEMESTER_START`  | `str`  | 开课日          | `2026-03-02`                                                                                                                             | 格式为`YYYY-MM-DD`      |
| `USE_WEBVPN`      | `bool` | 使用webvpn     | `True`                                                                                                                                   | 如果连接了校园网，建议改为`False` |
| `TIME_LIST`       | `list` | 课程表时间        | `['08:30', '09:15', '10:15', '11:00', '11:45','14:00', '14:45', '15:45', '16:30', '17:15','19:00', '19:40', '20:30', '21:10', '18:00',]` | 一般无需更改               |
| `JWXT_URL`        | `str`  | 内网教务系统网址     | `https://jwxt.sztu.edu.cn/`                                                                                                              | 一般无需更改               |
| `JWXT_URL_WEBVPN` | `str`  | WebVPN教务系统网址 | `https://jwxt-sztu-edu-cn-s.webvpn.sztu.edu.cn:8118/`                                                                                    | 一般无需更改               |



>双击start.bat即可操作使用（使用Anaconda注意激活环境）

### 3.打包成exe文件（可选）
>执行build.bat即可打包成exe文件。（使用Anaconda注意激活环境）

### 4. 大功告成！ヾ(@^▽^@)ノ
（觉得好用不妨点个star😋。本人代码能力还有待提高，若遇到bug莫责怪o(╥﹏╥)o）

### 5. BUGS
- 多次密码/验证码错误会导致账号被锁定，建议使用前先确认密码正确。
- 多次密码/验证码错误会导致需要输入图形验证码，当前版本不支持图形验证码登录，建议使用前先确认密码正确。(在本机登录成功过一次后，应该不会再出现图形验证码了)
- 如遇到问题，请携带详细错误信息或截图(debug.png)提交issue。

## ~~Github Actions 自动构建~~
~~基于Github Actions的构建功能，打包成exe文件供直接下载~~  
由于打包文件过大，已没有必要继续使用该功能。需要打包成exe文件请使用build.bat自行打包。  
最后一次打包记录：[SZTU_course_xls2ics-Windows](https://github.com/TigGrak/SZTU_course_.xls_TO_.ics/actions/runs/19558836599/artifacts/4636121301)
